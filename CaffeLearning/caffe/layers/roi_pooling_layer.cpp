...
void ROIPoolingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  // RoI pooling层输入参数pooled_h，pooled_w和spatial_scale
...
void ROIPoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const Dtype* bottom_data = bottom[0]->cpu_data(); // 获取share conv特征图数据
  const Dtype* bottom_rois = bottom[1]->cpu_data(); // 获取rois区域建议层数据
  // Number of ROIs
  int num_rois = bottom[1]->num(); // rois区域的数量
  int batch_size = bottom[0]->num(); // 输入图像batch
  int top_count = top[0]->count(); // 输出top的总容量
  Dtype* top_data = top[0]->mutable_cpu_data();
  caffe_set(top_count, Dtype(-FLT_MAX), top_data); // 初始化top数据
  int* argmax_data = max_idx_.mutable_cpu_data(); // 记录pooling过程中max indices
  caffe_set(top_count, -1, argmax_data);

  // For each ROI R = [batch_index x1 y1 x2 y2]: max pool over R
  for (int n = 0; n < num_rois; ++n) {
    int roi_batch_ind = bottom_rois[0];
    int roi_start_w = round(bottom_rois[1] * spatial_scale_); // roi相对于原图，share conv相对于原图为spatical_scale_，比如1/16，映射到当前的feature map
    ...
    const Dtype* batch_data = bottom_data + bottom[0]->offset(roi_batch_ind); // 获得batch偏移后的数据
    ...
    for (int c = 0; c < channels_; ++c) {
      for (int ph = 0; ph < pooled_height_; ++ph) {
        for (int pw = 0; pw < pooled_width_; ++pw) {
          ...
          hstart = min(max(hstart + roi_start_h, 0), height_); // 限制宽度和高度
